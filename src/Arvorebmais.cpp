 ...
 **/
void BPlusTreeInt::insertNonFull(No *node, int key, long dataPointer)
{
    // ==========================================================
    ...
    if (node->ehFolha)
    {
        ...
        auto it = std::lower_bound(node->vetorChaves.begin(), node->vetorChaves.end(), key);
        int i = std::distance(node->vetorChaves.begin(), it);

        // Insere a chave
        node->vetorChaves.insert(node->vetorChaves.begin() + i, key);
        // Insere o ponteiro de DADO
        node->vetorApontadores.insert(node->vetorApontadores.begin() + i, dataPointer);
        node->numChaves++;
    ...
    else
    {
        ...
        auto it = std::upper_bound(node->vetorChaves.begin(), node->vetorChaves.end(), key);
        int i = std::distance(node->vetorChaves.begin(), it);
        ...
        long childOffset = node->vetorApontadores[i];
        No *childNo = new No();
        ...
        if (childIsFull)
        {
            ...
            childOffset = node->vetorApontadores[i];
            lerNo(childOffset, childNo);
        }
